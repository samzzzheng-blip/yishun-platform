package com.techtron.onebook.module.infra.service.codegen.inner;
...
import com.techtron.onebook.framework.apilog.core.enums.OperateTypeEnum;
...
import com.techtron.onebook.framework.common.util.date.DateUtils;
...
import com.techtron.onebook.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.techtron.onebook.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.techtron.onebook.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.techtron.onebook.module.infra.enums.codegen.CodegenSceneEnum;
import com.techtron.onebook.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.techtron.onebook.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.techtron.onebook.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 onebook-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 onebook-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.techtron.onebook.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/onebook-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/onebook-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "onebook-module-${table.moduleName}/" + // 顶级模块
                "onebook-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "onebook-module-${table.moduleName}/" + // 顶级模块
                "onebook-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "onebook-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "onebook-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "onebook-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
