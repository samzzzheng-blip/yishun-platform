...
// 本地安全预览入口：只在 .env.local 显式启用，不会进入生产构建。
if (import.meta.env.DEV && import.meta.env.VITE_LOCAL_PREVIEW === 'true') {
  remainingRouter.unshift({
    path: '/collection',
    component: Layout,
    name: 'LocalCollectionPreview',
    redirect: '/collection/yikoujia',
    meta: {
      title: '藏品系统',
      icon: 'ep:suitcase',
      alwaysShow: true
    ...
    children: [
      {
        path: 'yikoujia',
        component: () => import('@/views/app/yikoujia/index.vue'),
        name: 'LocalYikoujiaPreview',
        meta: {
          title: '一口价管理',
          icon: 'ep:price-tag',
          noCache: true
