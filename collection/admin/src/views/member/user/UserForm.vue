...
function validateAccount(rule: any, value: string, callback: any) {
  const accountRegex = /^[a-zA-Z][a-zA-Z0-9]{3,11}$/
  if (!accountRegex.test(value)) {
    callback(new Error('账号格式不正确，只能包含字母和数字,长度在4-12之间,且以字母开头'))
  } else {
    callback()
  }
}
...
function validatePassword(rule: any, value: string, callback: any) {
  const passwordRegex = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]+\S{5,12}$/
  if (!passwordRegex.test(value)) {
    callback(new Error('密码格式不正确，长度在6-12之间，且包含字母和数字'))
  } else {
    callback()
  }
}
