<template>
  <el-container class="kefu-layout">
    ...
    <KeFuConversationList
      v-show="screenWidth > 1000 || showLeftSide"
      ref="keFuConversationRef"
      @change="handleChange"
    />
    <!-- 会话详情（选中会话的消息列表） -->
    <KeFuMessageList
      v-show="screenWidth > 1000 || !showLeftSide"
      ref="keFuChatBoxRef"
      @show-left-side="showLeftSide = true"
    />
    <!-- 会员信息（选中会话的会员信息） -->
    <MemberInfo v-if="screenWidth > 1000" ref="memberInfoRef" />
  </el-container>
</template>
...
const showLeftSide = ref(true) // 是否显示左侧会话列表
