<template>
  ...
  <ContentWrap>
    ...
    <el-table-column type="selection" width="55" />
      ...
      <el-table-column label="实际成交价格" align="center" prop="dealPrice" :formatter="fenToYuanFormat" />
      <el-table-column
        label="创建时间"
        align="center"
        prop="createTime"
...
import { BuyOrderApi, BuyOrder } from '@/api/app/buyorder'
import { fenToYuanFormat } from '@/utils/formatter'

/** 批量交易买单 列表 */
defineOptions({ name: 'BuyOrder' })



const loading = ref(true) // 列表的加载中
const list = ref<BuyOrder[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await BuyOrderApi.getBuyOrderPage(queryParams)
