...
function load() {
  checkUpdate();
  // 提前加载订阅模板。真正拉起微信授权必须由用户点击触发，不能在这里直接请求。
  getSubscribeTemplate().catch((error) => {
    console.log('[subscription] preload templates failed:', error);
  });
}
...
// 订阅消息
async function subscribeMessage(event, callback = undefined) {
  if (subscribeEventList.length === 0) {
  ...
  tmplIds = [...new Set(tmplIds)].slice(0, 3);
  if (tmplIds.length === 0) return {};

  return requestSubscribeTemplateIds(tmplIds, callback);
}

function requestSubscribeTemplateIds(tmplIds, callback = undefined) {
  const ids = [...new Set((tmplIds || []).filter(Boolean))].slice(0, 3);
