...
<script>
  ...
  export default {
    ...
    created() {
      if (this.driver === 'local' && typeof uniCloud !== 'undefined') {
    ...
    methods: {
      ...
      uploadFiles(files) {
        if (typeof uniCloud === 'undefined') {
          console.warn('当前项目未关联 uniCloud，无法使用云空间上传');
      ...
      async getTempFileURL(fileList) {
        if (typeof uniCloud === 'undefined') {
          return Array.isArray(fileList) ? fileList[0] || '' : fileList || '';
        }
        fileList = {
...
<style lang="scss" scoped>
  ...
  .is-add1 {
    width: 168rpx;
    height: 168rpx;
    background: url('https://ysyz2025.oss-cn-hangzhou.aliyuncs.com/static/add.png') no-repeat;
    background-size: 100%;
	margin-top:10px;
  }
  .add-icon {
