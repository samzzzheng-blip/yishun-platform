<template>
  <view
    class="page-app"
    :class="['theme-' + (dark ? 'dark': 'light'), 'main-' + sys?.theme, 'font-' + sys?.fontSize]"
  >
    <view class="page-main" :style="[bgMain]">
      <!-- 顶部导航栏-情况1：默认通用顶部导航栏 -->
      <view v-if="navbar === 'normal' && sheep.$platform.name !== 'H5'" class="page-navbar">
      <su-navbar
        fixed
        :title="title"
        statusBar
      ...
      <view class="page-body" :style="[bgBody]">
        <!-- 顶部导航栏-情况3：沉浸式头部 -->
        <su-inner-navbar v-if="navbar === 'inner' && sheep.$platform.name !== 'H5'" :title="title" />
...
<script setup>
  ...
  const safeBottomHeight = computed(() => sheep.$platform.device.safeAreaInsets.bottom);
  ...
  .page-app {
    ...
    .page-main {
      ...
      .page-body {
        ...
        flex: 1;
        overflow-y: scroll;
        // 不对滚动容器使用 transform/fill-mode:both，避免改变内部固定元素的参照系。
      }
    }
  }
