...
const app = defineStore({
  ...
  state: () => ({
    ...
    collectionTabTarget: null, // 从其他底部导航页进入藏品页时，一次性指定目标页签
  }),
  actions: {
    // 获取Shopro应用配置和模板
	...
	async updateMessgaeNum(clean = false) {
    if (clean) {
      this.template.basic.tabbar.items[3].badge = null
      return;
    }
		let { code, data } = await tradeApi.getNotify({readStatus: false,pageSize: 1,
          pageNo: 1,})
		if (code !== 0) {
      this.template.basic.tabbar.items[3].badge = null
		  return;
		}
		this.template.basic.tabbar.items[3].badge = data.total > 0 ? data.total : null
