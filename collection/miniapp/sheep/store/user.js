...
const user = defineStore({
  ...
  state: () => ({
    ...
    tradeInfo: {}, // 交易相关信息
    storageCapacity: {}, // 空间容量信息
  }),

  actions: {
    // 获取交易相关信息
    async getTradeInfo(clean = false) {
      if (clean) {
        this.tradeInfo = {};
        return;
      }
      sellApi.getUserTradeInfo().then(res => {
        this.tradeInfo = res.data
      })
    },
    // 获取空间容量信息
    async getStorageCapacity() {
      collectionApi.getStorageCapacity().then(res => {
        if (res.code === 0) {
          this.storageCapacity = res.data
        }
      })
    },
    // 获取用户信息
