...
export default class LeftNav extends Component {
      ...
      if (!item.children || item.children.length === 0) {
        return (
          <Menu.Item key={item.resKey ? item.resKey : item.id} name={item.resName} style={{ paddingLeft: 0 }}>
            <Icon type={`${item.resIcon}`} title={item.resName} />
  ...
  render() {
    return (
      <div className={this.state.isLeftNavMini&&!this.props.mobileExpanded ? 'LeftNavMini' : ''}>
        <nav id="mainnav-container" className="mainnav-container">
          ...
          <Spin spinning={false}>
            <Menu onClick={this._handleClick}
              ...
              mode="inline"
              inlineIndent="16"
              inlineCollapsed={this.props.mobileExpanded?false:this.state.collapsed}
            >
              {this.renderLeftNav()}
            </Menu>
          </Spin>
