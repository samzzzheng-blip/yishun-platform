    ...
    this.props.form.validateFields((errors, values) => {
      ...
      this.setState({ loading: true }, () => {
        if (this.props.type === 'edit') {
          ...
          postData('updatePrecious', params).then((data) => {
          ...
          // 新增
          postData('addPrecious', values).then((data) => {
    ...
    return (
      ...
      >
        <Form onSubmit={this.handleSubmit}>
          ...
          <FormItem {...formItemLayout} label="签名人" hasFeedback>
            {getFieldDecorator('signer', {
              initialValue: values ? values.signer : '',
              rules: [
                { required: true, message: '请输入签名人' },
              ],
            })(<Input placeholder="请输入签名人" maxLength={50} />)}
          </FormItem>
          ...
          <FormItem {...formItemLayout} label="卡品" hasFeedback>
            {getFieldDecorator('publishActivity', {
              initialValue: values ? values.publishActivity : '',
              rules: [
                { required: false, message: '请输入卡品' },
              ],
            })(<Input placeholder="请输入卡品" maxLength={100} />)}
          </FormItem>
          <FormItem {...formItemLayout} label="活动地址" hasFeedback>
            {getFieldDecorator('publishCity', {
              initialValue: values ? values.publishCity : '',
              rules: [
                { required: false, message: '请输入活动地址' },
              ],
            })(<Input placeholder="请输入活动地址" maxLength={50} />)}
          </FormItem>
          <FormItem {...formItemLayout} label="活动时间" hasFeedback>
            {getFieldDecorator('publishTime', {
              initialValue: values ? values.publishTime : '',
              rules: [
                { required: false, message: '请选择活动时间' },
              ],
            })(<Input placeholder="请输入活动时间" maxLength={50} />)}
          </FormItem>
          <FormItem {...formItemLayout} label="亲笔签名" hasFeedback>
            {getFieldDecorator('publishSign', {
              initialValue: values ? values.publishSign : '',
              rules: [
                { required: false, message: '请输入亲笔签名' },
              ],
            })(<Input placeholder="请输入亲笔签名" maxLength={100} />)}
          </FormItem>
