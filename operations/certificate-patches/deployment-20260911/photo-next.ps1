...
$work='D:\deploy\grading-workflow-photo-next-20260911'
$jar='D:\project\yishun-0.0.1-SNAPSHOT.jar'
$patch='D:\home\usr_upload\tmp\20260911\1789112198866_773.zip'
if(Test-Path $work){throw 'Release directory already exists; do not rerun'}
if((Get-FileHash $patch -Algorithm SHA256).Hash -ne '2cde38a4e7a1eff0e11386b91b7270d3dfe243d8bf9655ccb99954eff00e4b32'){throw 'Checksum mismatch'}
...
try {
 ...
 foreach($asset in @('client.978a.js','common.978a.js','style.978a.css')){Copy-Item "$work\patch\frontend\$asset" "D:\dist\$asset"}
 Copy-Item "$work\patch\frontend\index.html" 'D:\dist\index.html' -Force
 Set-Content "$work\DEPLOY_OK" (Get-Date -Format o)
 Write-Output ('PHOTO_NEXT_OK PID='+$new.Id)
}catch{
