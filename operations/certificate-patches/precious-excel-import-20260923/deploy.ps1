...
$stage='D:\deploy\precious-import-20260923-v2'
...
foreach($item in $manifest.files|Where-Object {$_.path -like 'frontend/*' -and $_.path -ne 'frontend/index.html'}){
 $dest=Join-Path 'D:\dist' ([IO.Path]::GetFileName($item.path));if(Test-Path $dest){throw 'Asset name already exists'};Copy-Item (Join-Path "$stage\payload" $item.path) $dest
}
...
try{
 ...
 Launch 'new'
 Copy-Item "$stage\payload\frontend\index.html" 'D:\dist\index.precious-import.pending'
 [IO.File]::Replace('D:\dist\index.precious-import.pending',$index,"$backup\index-switch.html")
