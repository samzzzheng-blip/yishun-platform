...
$newClient='D:\dist\client.precious-20260918.js'
...
if((Test-Path $newClient) -and (Hash $newClient) -ne ($manifest.files | Where-Object path -eq 'frontend/client.precious-20260918.js').sha256){throw 'New frontend filename exists with unexpected contents.'}
...
if((Hash 'D:\dist\client.b066.js') -ne $manifest.baseline.client){throw 'Online frontend changed since inspection.'}
...
if(!(Test-Path $backup)) {
  ...
  Copy-Item 'D:\dist\client.b066.js' (Join-Path $backup 'client.b066.js')
}
...
# Verify every archive entry. Only the three declared classes may differ.
...
try {
  ...
  Copy-Item (Join-Path $stage 'payload\frontend\client.precious-20260918.js') $newClient
  Copy-Item (Join-Path $stage 'payload\frontend\index.html') 'D:\dist\index.precious-20260918.pending'
  [IO.File]::Replace('D:\dist\index.precious-20260918.pending',$liveIndex,(Join-Path $stage ('index-before-switch-'+[guid]::NewGuid().ToString('N')+'.html')))
  if((Hash $newClient) -ne ($manifest.files | Where-Object path -eq 'frontend/client.precious-20260918.js').sha256){throw 'Installed frontend hash mismatch.'}
